...
if( class_exists( 'WP_Customize_Control' ) && !class_exists( 'DOCENT_PRO_THMC_Radio_Image_Control' ) ):
	/**
	* 
	*/
	class DOCENT_PRO_THMC_Radio_Image_Control extends WP_Customize_Control
	{
			...
			<div class="thmc-radio-images clearfix">
				<?php foreach ($this->choices as $input_value => $img_src): ?>
					<div class="thmc-radio-image">
						...
						<label for="<?php echo esc_attr( $name.'_'.$input_value ); ?>"><img src="<?php echo esc_url( $img_src ); ?>" /></label>
					</div>
				<?php endforeach;?>
			</div>
			<?php
